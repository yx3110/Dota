

----------------------------------------------------------------------------------------------------

function Think()

	--[[if( GetGameMode()==GAMEMODE_1V1MID)
	then
		if(GetTeam() == TEAM_RADIANT) then
			print("Selecting SF for radiant")
			SelectHero( 2, "npc_dota_hero_nevermore" );
		elseif(GetTeam() == TEAM_DIRE) then
			print("Selecting SF for Dire")
			SelectHeor( 7, "npc_dota_hero_nevermore");
		end
	else--]]
	if ( GetTeam() == TEAM_RADIANT ) then
		print( "selecting radiant" );
		SelectHero( 2 , "npc_dota_hero_antimage" );
		SelectHero( 3 , "npc_dota_hero_drow_ranger" );
		SelectHero( 4 , "npc_dota_hero_sven" );
		SelectHero( 5 , "npc_dota_hero_undying" );
		SelectHero( 6 , "npc_dota_hero_crystal_maiden" );
	elseif ( GetTeam() == TEAM_DIRE ) then
		print( "selecting dire" );
		SelectHero( 7 , "npc_dota_hero_lina" );
		SelectHero( 8 , "npc_dota_hero_earthshaker" );
		SelectHero( 9 , "npc_dota_hero_juggernaut" );
		SelectHero( 10 , "npc_dota_hero_mirana" );
		SelectHero( 11 , "npc_dota_hero_nevermore" );
	end
end
--end

----------------------------------------------------------------------------------------------------
